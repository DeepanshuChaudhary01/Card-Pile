...
import { addCard } from '../features/Reducer_bucket';

const All_cards = ({cards, bucketIndex, rerenderOnce}) => {
  ...
  return (
    <Box sx={{ position: 'relative', flexGrow: 1, p: '0.5rem', display: 'flex', flexDirection: 'column' }}>
      <Grid sx={{ height: '100%' ,mt: '0.5rem', p: '1rem', alignContent: 'start' }} container rowSpacing={3} columnSpacing={{ xs: 1, sm: 2, md: 3 }}>
        ...
        {
            bucketIndex !== -1 &&
          <Fab onClick={()=>handleAddcard(bucketIndex)} color="primary" aria-label="add"  sx={{ position: 'relative', top: '3.1rem', left: '2.5rem' }} >
            <Add />
          </Fab>
        }
      </Grid>
